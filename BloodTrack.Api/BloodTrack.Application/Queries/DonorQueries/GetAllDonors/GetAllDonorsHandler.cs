...
{
    public class GetAllDonorsHandler : IRequestHandler<GetAllDonorsQuery, ResultViewModel<List<GetAllDonorsViewModel>>>
    {
        ...
        public async Task<ResultViewModel<List<GetAllDonorsViewModel>>> Handle(GetAllDonorsQuery request, CancellationToken cancellationToken)
        {
            var donations = await _repository.GetAll();

            if (donations is null)
            {
                return ResultViewModel<List<GetAllDonorsViewModel>>.Error("Não existem doadores");
            }
